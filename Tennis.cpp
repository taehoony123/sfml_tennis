...
// Function to get the resource directory (for loading assets)
std::string resourcesDir() {
    // Adjust this path according to your resource directory
    return "resources/";
}
    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event))
        {
                ...
                (event.type == sf::Event::TouchBegan))
            {
                if (!isPlaying)
                {
                    ...
                    // Reset the ball speed and angle
                    ballSpeed = initialBallSpeed + (leftScore + rightScore) * 50; // Include current scores in the speed
                    do
                    {
        ...
        if (isPlaying)
        {
            ...
            if (ball.getPosition().x - ballRadius < 0.f)
            {
                isPlaying = false;
                rightScore++;
                ballSpeed = initialBallSpeed + (leftScore + rightScore) * 50; // Increase ball speed based on combined score
                if (rightScore >= 5)
                {
                ...
                {
                    pauseMessage.setString("Right Player Scores!\n\n" + inputString);
                }
            }
            if (ball.getPosition().x + ballRadius > gameWidth)
            {
                ...
                {
                    pauseMessage.setString("Left Player Scores!\n\n" + inputString);
                }
            }
